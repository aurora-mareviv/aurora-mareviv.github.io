...
# The <span style="font-family: monospace">blogdown</span> R package &nbsp; <span class="fa fa-newspaper-o"></span>

Finally, -after 24h of failed attempts-, I could get my favourite [Hugo theme](https://github.com/kakawait/hugo-tranquilpeak-theme) up and running with R Studio and [Blogdown](https://bookdown.org/yihui/blogdown/).   
...
I was able to install the Hugo Yihui's [lithium theme](https://github.com/yihui/hugo-lithium-theme) in a new repository.  
...
- In my machine, I installed Blogdown & Hugo from R studio (v 1.1.336) the usual way. 
- In GiHub, it was easier for me to host the blog directly in my main github pages repository (always named <span style="font-family: monospace">[USERNAME].github.io</span>), in the <span style="font-family: monospace">master</span> branch. 
- I have basic knowledge of html, css and javascript, so I didn't mind to tinker around with the theme.
- However my custom styles didn't involve theme [rebuilding](https://github.com/kakawait/hugo-tranquilpeak-theme/blob/master/docs/user.md#quick--easy-modifications), as at this moment they're simple cosmetic tricks. 

The steps I followed were: 

- <span class="fa fa-github"></span> Setting a [GitHub](https://github.com/) repo with the name <span style="font-family: monospace">[USERNAME].github.io</span> (in my case <span style="font-family: monospace">aurora-mareviv.github.io</span>). See [this](https://help.github.com/articles/create-a-repo/) and [this](https://tclavelle.github.io/blog/blogdown_github/).  
- Create a `git` repo in your machine:
    + Create manually a new directory called <span style="font-family: monospace">[USERNAME].github.io</span>. 
    + Run in the terminal:  
...
- For now, your repo is ready. We will focus now in creating & customising our Blogdown.
- We will open RStudio (v 1.1.336, [development](https://www.rstudio.com/products/rstudio/download/preview/) version as of today). 

    + First, you may need to install bookdown in R:
...
![Selecting Hugo Blogdown format](/images/tuto/tuto3.png)
...
Before we <span style="font-family: monospace">build</span> and <span style="font-family: monospace">serve</span> our site, we need to tweak a couple of things in advance, if we want to smoothly deploy our blog into GitHub pages.  

### Modify <span style="font-family: monospace">config.toml</span> file
...
- By default, the web page is published in the "public" folder. We need it to be published in the root of the repository, to match the structure of the GitHub <span style="font-family: monospace">master</span> branch:  
...
The cover (background) image must be located in <span style="font-family: monospace"> /themes/hugo-tranquilpeak-theme/static/images </span>. To modify it in the *config.toml*:  

```{r eval=FALSE}
coverImage = "myimage.jpg"
```

We want some custom css and js. We need to locate it in <span style="font-family: monospace"> /static/css </span> and in <span style="font-family: monospace"> /static/js </span> respectively.
...
Since I started writing in Bootstrap, I miss it a lot. Since this theme already has bootstrap classes, I brought some others I didn't find in the theme (okay, they're [available](https://github.com/kakawait/hugo-tranquilpeak-theme/blob/master/docs/user.md) for *.md* files, but currently not for *.Rmd*)
...
<div id="welcome" class="jumbotron text-center alert-info"><h1>Welcome to the era of reproducible blogging!</h1></div>
