...
- To ensure reproducibility within a stable working directory tree. She proposes the very concise `here::here()` but other methods are available such as the `template` [package](https://github.com/Pakillo/template).   
...
I also added two more features to the tree:  

- An `.Rproj` file
- A <span class="texttt">Git</span> repository so the tree can be conveniently cloned or downloaded:   
...
```{r comment="  ", message=FALSE, warning=FALSE, fig.height=4, fig.width=5}
...
```{r comment="  ", eval=FALSE, message=FALSE, warning=FALSE}
# TO PDF 
plotname1 <- "p1.pdf"
# TO PNG 
plotname2 <- "p1.png"
...
```{r  comment="  ", eval=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
# MSEXCEL # not run
